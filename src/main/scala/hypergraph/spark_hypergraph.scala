...
object spark_hypergraph extends Serializable {
  ...
  */
  ...
  /**
    ...
    * @return
    */
  def start_hypergraph(n: Int, t: Int, v: Int, sc: SparkContext, seed: Long): Array[Array[Char]] = {

    val expected = numberTWAYCombos(n, t, v)
    println("Apache Spark Hypergraph Vertex Cover")
    println("Using Greedy Picker heuristic")
    println("Using OX algorithm + BitSets for Fast delete")
    println(s"Seed: $seed")
    ...
    val tests = greedy_algorithm(sc, v, combos)

    println(s"ts size : ${tests.size}")
    var t2 = System.nanoTime()
    var time_elapsed = (t2 - t1).toDouble / 1000000000

    pw.append(s"$t;$n;$v;SPARK_HYPERGRAPH;seed=$seed;$time_elapsed;${tests.size}\n")
    println(s"$t;$n;$v;SPARK_HYPERGRAPH;seed=$seed;$time_elapsed;${tests.size}\n")
    pw.flush()
